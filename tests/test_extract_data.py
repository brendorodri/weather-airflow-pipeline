...
def test_extract_weather_data_success(mocker):
    ...
    mock_get = mocker.patch('extract_data.requests.get')
    mock_json_dump = mocker.patch('extract_data.json.dump')
    mock_open_file = mocker.patch('extract_data.open')
    mock_mkdir = mocker.patch('extract_data.Path.mkdir')
